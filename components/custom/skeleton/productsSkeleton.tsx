...
import { ProductCardSkeletonV1 } from "./productCardSkeleton";

export function ProductSkeletonV1() {
  return (
    <section className="animate-pulse">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
        {[...Array(5)].map((_, i) => (
            <ProductCardSkeletonV1 key={i} i={i} />
        ))}
      </div>
    </section>
  );
...
export function ProductSkeletonV2() {
  return (
    <section className="animate-pulse">
      {/* Header matches: ShoppingBag w-4 h-4 + text-base font-semibold + "See all" Button */}
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <div className="w-4 h-4 bg-muted rounded" />
          <div className="h-[15px] w-36 bg-muted rounded" />
        </div>
        <div className="h-[13px] w-12 bg-muted rounded-full" />
      </div>

      {/* Grid matches your exact breakpoints */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
        {[...Array(5)].map((_, i) => (
          <div
            key={i}
